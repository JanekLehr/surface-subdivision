...
std::ostream &operator << (std::ostream &s, const TMat4 &m)
{
	...
	return(s << '[' << m[0] << std::endl << std::setw(w) << m[1] << std::endl
		<< std::setw(w) << m[2] << std::endl << std::setw(w) << m[3] << ']' << std::endl);
}
...
std::istream &operator >> (std::istream &s, TMat4 &m)
{
    ...
    if (c == '[')			
    {
		...
		if (!s)
		{
			std::cerr << "Expected number while reading matrix\n";
			return(s);
		}
		...
		if (c != ']')
    	{
			...
			std::cerr << "Expected ']' while reading matrix\n";
	    	return(s);
    	}
	}
    else
